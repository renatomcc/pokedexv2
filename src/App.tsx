...
const noData = [{ value: "none", label: "none" }];
const IDNameData = [
  { value: "id", label: "ID" },
...
function App() {
  ...
  var pokelist: Pokemon[] =
    searchPoke.length > 0
      ? filteredDex
          .filter((poke) =>
            poke.name.toLocaleLowerCase().includes(searchPoke.toLowerCase())
          )
          .slice(0, pokesPerPage)
      : filteredDex.slice(0, pokesPerPage);
  const [scroll, scrollTo] = useWindowScroll();

  async function fetchData() {
    Promise.all(
      await axios
        .get("https://pokeapi.co/api/v2/pokemon/?limit=858")
        .then((data: any) => {
      ...
      .then((result) => {
        result.map(async (item) => {
          ...
          axios.get(`${item.species.url}`).then((res) => {
            if (res.data.habitat)
              pokemon.habitat =
                res.data.habitat.name[0].toUpperCase() +
                res.data.habitat.name.slice(1);
            if (res.data.shape)
              pokemon.shape =
                res.data.shape.name[0].toUpperCase() +
                res.data.shape.name.slice(1);
            ...
            if (
              !res.data.is_baby &&
              !res.data.is_mythical &&
              !res.data.is_legendary
            )
  ...
  function handleSelect(selectOption: string | null) {
    ...
    switch (selectOption) {
      case "favorites":
        setData(noData);
        setFilteredDex(
          filteredDex.filter((poke) => favorites.includes(poke.id))
        );
        break;
      case "id":
        setData(IDNameData);
      ...
      case "type":
        if (filterOption == "all") setFilteredDex(pokemons);
          ...
          setFilteredDex(
            pokemons.filter(
              (poke) =>
                poke.types[0].type.name == filterOption ||
                (poke.types[1] != null &&
                  poke.types[1].type.name == filterOption)
            )
          );
        break;
      case "category":
        setFilteredDex(
          pokemons.filter((poke) => poke.category == filterOption)
        );
        break;
      case "habitat":
        if (filterOption == "all") setFilteredDex(pokemons);
          ...
          setFilteredDex(
            pokemons.filter((poke) => poke.habitat == filterOption)
          );
        break;
      case "shape":
        if (filterOption == "all") setFilteredDex(pokemons);
        else
          setFilteredDex(pokemons.filter((poke) => poke.shape == filterOption));
        break;
    }
  }
    ...
    >
        ...
        <StyledColumn xs={12} sm={8} md={8} lg={5} xl={5}>
          <StyledGrid>
            <StyledColumn style={{ flexDirection: "column" }}>
              <StyledBadge
                color={darkTheme ? "indigo" : "dark"}
                radius="sm"
                variant={darkTheme ? "light" : "filled"}
                size="lg"
              >
                SORT BY
              </StyledBadge>
      ...
      {isLoading && (
        <>
          ...
          <Badge color="pink" size="xl" radius="xs" variant="filled">
            Catching 'Em All!
          </Badge>
        </>
      )}
        ...
        <>
          ...
          {pokesPerPage <= filteredDex.length ? (
            <Button
              ...
              radius="md"
              size="md"
              leftIcon={<MdCatchingPokemon size={24} />}
              onClick={() => loadMore()}
            >
              Catch more!
            </Button>
...
const _StyledBadge = styled(Badge)`
  margin: 10px 0px;
`;
const StyledBadge = createPolymorphicComponent<"button", BadgeProps>(
  _StyledBadge
);
